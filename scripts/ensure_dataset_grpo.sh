...
#
# Called automatically at the top of GRPO entry scripts (grpo_train.sh,
# train.sh, lite_grpo_test.sh, lite_e2e_benchmark.sh, test_dtypes.sh).
...
# (~28 GB, resumable) into <root>/dataset_zips/, then restore the exact
# Train/Validation/Test layout with dataset_grpo/unzip_dataset.py.
#
# Env:
...
mkdir -p "$ZIP_DIR"
log "downloading zips -> $ZIP_DIR (resume-safe, ~28 GB total) ..."
...
UNZIP_PY="$REPO_ROOT/dataset_grpo/unzip_dataset.py"
[ -f "$UNZIP_PY" ] || UNZIP_PY="$REPO_ROOT/dataset_sft/unzip_dataset.py"
[ -f "$UNZIP_PY" ] || err "unzip_dataset.py not found in dataset_grpo or dataset_sft — fresh clone incomplete?"
log "extracting -> $ROOT ..."
